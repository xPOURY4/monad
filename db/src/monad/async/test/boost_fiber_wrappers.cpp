...
namespace
{
    ...
    TEST(BoostFiberWrappers, fiber_read)
    {
    ...
    TEST(BoostFiberWrappers, fiber_timeout)
    {
        auto impl = []() -> std::chrono::steady_clock::duration {
            auto begin = std::chrono::steady_clock::now();
            boost_fibers::timed_delay(*testio, std::chrono::seconds(1))
                .get()
                .value();
    ...
    TEST(BoostFiberWrappers, resume_execution_upon)
    {
        ...
        auto impl = [&] {
            ...
            // race.
            boost_fibers::resume_execution_upon(*testio).get().value();
