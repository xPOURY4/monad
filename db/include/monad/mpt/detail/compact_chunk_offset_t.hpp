...
namespace detail
{
    ...
    public:
        constexpr compact_chunk_offset_t(chunk_offset_t const offset)
            : v_{static_cast<uint32_t>(offset.raw() >> bits_to_truncate)}
        {
        }

        constexpr compact_chunk_offset_t(uint32_t v)
            : v_{v}
        {
        }
        ...
        constexpr compact_chunk_offset_t
        operator-(compact_chunk_offset_t const &o) const noexcept
        {
            return compact_chunk_offset_t(v_ - o.v_);
        }

        constexpr compact_chunk_offset_t &
        operator+=(compact_chunk_offset_t const &o)
        {
            v_ += o.v_;
            return *this;
        }
    ...
    static_assert(sizeof(compact_chunk_offset_t) == 4);
    static_assert(alignof(compact_chunk_offset_t) == 4);
}
