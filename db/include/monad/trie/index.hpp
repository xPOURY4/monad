...
public:
    ...
    constexpr file_offset_t get_start_offset()
    {
        return BLOCK_START_OFF;
    }

    file_offset_t get_history_root_off(uint64_t vid)
    {
        ...
        auto info = reinterpret_cast<block_trie_info *>(
            buffer.get() + record_off - offset);
        MONAD_ASSERT(info->vid == vid);
    ...
    void write_record(uint64_t vid, uint64_t root_off)
    {
        ...
        if (MONAD_UNLIKELY(!mmap_block_)) {
            // initial mmap for non-header block
            block_start_off_ = round_down_align<CPU_PAGE_BITS>(record_off);
            if (block_start_off_) { // not the header block
                mmap_block_ = _memmap(block_start_off_);
            }
        }
        else if (record_off >= block_start_off_ + PAGE_SIZE) {
            // renew mmap_block_ to the next block
            if (mmap_block_) {
                MONAD_ASSERT(!munmap(mmap_block_, CPU_PAGE_SIZE));
            }
        ...
        *reinterpret_cast<uint64_t *>(header_block_) = vid;
        MONAD_ASSERT(!msync(write_block, CPU_PAGE_SIZE, MS_ASYNC));
    }
};
