...
namespace test_resource
{
    ...
    inline std::filesystem::path const vm_data_dir = "@VM_DATA_DIR@";

    // blocks
    inline auto const correct_block_data_dir =
        test_data_dir / "data" / "blocks" / "compressed_blocks";
    inline auto const bad_decompress_block_data_dir =
        test_data_dir / "data" / "blocks" / "bad_decompress_blocks";
    inline auto const bad_decode_block_data_dir =
        test_data_dir / "data" / "blocks" / "bad_decode_blocks";
    inline auto const self_compressed_block_data_dir =
        test_data_dir / "data" / "blocks" / "self_compressed_blocks";

    // ethereum tests
    inline auto const ethereum_tests_dir = third_party_dir / "ethereum-tests";
    inline auto const internal_blockchain_tests_dir = test_data_dir / "data" / "shared";

    inline auto const checkpoint_dir = test_data_dir / "data" / "checkpoint";

    // geth vectors
    inline auto const geth_vectors_dir = third_party_dir / "go-ethereum" /
                                         "core" / "vm" / "testdata" /
                                         "precompiles";

    // VM data
    inline auto const compile_benchmarks_dir =
        vm_data_dir / "compile_benchmarks";
    inline auto const execution_benchmarks_dir =
        vm_data_dir / "execution_benchmarks";
    inline auto const regression_tests_dir = vm_data_dir / "regression_tests";
}
...
inline auto const A_ICODE = vm::make_shared_intercode(A_CODE);
...
inline auto const B_ICODE = vm::make_shared_intercode(B_CODE);
    ...
    std::optional<std::vector<Withdrawal>> const &withdrawals = std::nullopt)
{
    ...
    // return the proposal block_id that has been finalized
    return block_id;
}
