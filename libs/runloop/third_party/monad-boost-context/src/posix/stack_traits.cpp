...
namespace boost
{
    namespace context
    {
        ...
        bool stack_traits::is_unbounded() BOOST_NOEXCEPT_OR_NOTHROW
        {
            return RLIM_INFINITY == stacksize_limit();
        }

        std::size_t stack_traits::page_size() BOOST_NOEXCEPT_OR_NOTHROW
        {
            static std::size_t size = pagesize();
            return size;
        }

        std::size_t stack_traits::default_size() BOOST_NOEXCEPT_OR_NOTHROW
        {
            return 128 * 1024;
        }

        std::size_t stack_traits::minimum_size() BOOST_NOEXCEPT_OR_NOTHROW
        {
            return static_cast<std::size_t>(MINSIGSTKSZ);
        }

        std::size_t stack_traits::maximum_size() BOOST_NOEXCEPT_OR_NOTHROW
        {
