...
impl std::fmt::Display for cxx_status_code_system {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            std::io::Error::from_raw_os_error(self.value as i32).to_string()
        )
    }
}

fn to_result(res: cxx_result_status_code_system_monad) -> MonadAsyncResult<isize> {
    if (res.flags & 1) == 1 {
        return Ok(res.value);
    }
    Err(res.error)
...
impl monad_context_switcher_ptr {
    #[allow(dead_code)]
    fn new(implem: &monad_context_switcher_impl) -> MonadAsyncResult<monad_context_switcher_ptr> {
...
impl monad_async_task_ptr {
    ...
    ) -> MonadAsyncResult<monad_async_task_ptr> {
