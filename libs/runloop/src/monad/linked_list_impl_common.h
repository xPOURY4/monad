...
{
#endif
static inline monad_context_cpu_ticks_count_t
get_ticks_count(MONAD_CONTEXT_CPP_STD memory_order rel)
{
    monad_context_cpu_ticks_count_t ret;
#if defined(__i386__) || defined(_M_IX86) || defined(__x86_64__) ||            \
    defined(_M_X64)
    unsigned lo, hi, aux;
    switch (rel) {
    case MONAD_CONTEXT_CPP_STD memory_order_acquire:
        __asm__ __volatile__("rdtsc\nlfence" : "=a"(lo), "=d"(hi));
        break;
    case MONAD_CONTEXT_CPP_STD memory_order_release:
        __asm__ __volatile__("mfence\nrdtscp\n"
                             : "=a"(lo), "=d"(hi), "=c"(aux));
        break;
    case MONAD_CONTEXT_CPP_STD memory_order_acq_rel:
    case MONAD_CONTEXT_CPP_STD memory_order_seq_cst:
        __asm__ __volatile__("mfence\nrdtscp\nlfence"
                             : "=a"(lo), "=d"(hi), "=c"(aux));
        break;
    default:
