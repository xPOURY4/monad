...
namespace
{
    ...
    TEST(AllocatorsTest, inline_owning_span)
    {
        using namespace MONAD_NAMESPACE::allocators;
        {
            auto v = inline_owning_span<int>(5, 2);
            ASSERT_EQ(v.size(), 5);
            for (auto const &i : v) {
                EXPECT_EQ(i, 2);
            }
        ...
        reset();
        {
            auto v = inline_owning_span<Foo, 1024, custom_allocator>(5, Foo(2));
            ASSERT_EQ(v.size(), 5);
            for (auto const &i : v) {
                EXPECT_EQ(i.x, 2);
            }
            ...
            auto v2(std::move(v));
            ASSERT_EQ(v2.size(), 5);
            ASSERT_NE(v2.data(), v_data);
            for (auto const &i : v2) {
                EXPECT_EQ(i.x, 2);
            }
            EXPECT_EQ(allocated, 0);
            EXPECT_EQ(constructed, 11);
            EXPECT_EQ(destructed, 6);
            EXPECT_EQ(deallocated, 0);
            v = std::move(v2);
            ASSERT_EQ(v.size(), 5);
            ...
            EXPECT_EQ(allocated, 0);
            EXPECT_EQ(constructed, 16);
            EXPECT_EQ(destructed, 11);
            EXPECT_EQ(deallocated, 0);
        }
        EXPECT_EQ(allocated, 0);
        EXPECT_EQ(constructed, 16);
