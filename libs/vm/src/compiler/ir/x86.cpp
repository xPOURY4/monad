...
using monad::compiler::basic_blocks::Instruction;
...
namespace
{
        ...
        int64_t instr_gas)
    {
        ...
        for (auto const &instr : block.instrs) {
            assert(remaining_base_gas >= instr.info().min_gas);
            remaining_base_gas -= static_cast<uint32_t>(instr.info().min_gas);
