...
[[nodiscard]] constexpr uint64_t g_data(Transaction const &txn) noexcept
{
    ...
    if constexpr (Traits::rev < EVMC_ISTANBUL) {
...
[[nodiscard]] constexpr uint64_t intrinsic_gas(Transaction const &txn)
{
    if constexpr (Traits::rev < EVMC_HOMESTEAD) {
        ...
        return 21'000 + g_data<Traits>(txn);
    }
    else if constexpr (Traits::rev < EVMC_BERLIN) {
        return 21'000 + g_data<Traits>(txn) + g_txn_create(txn);
    }
    else if constexpr (Traits::rev < EVMC_SHANGHAI) {
        return 21'000 + g_data<Traits>(txn) + g_txn_create(txn) +
...
{
    if constexpr (Traits::rev < EVMC_LONDON) {
        return gas_used * gas_price<Traits>(txn, base_fee_per_gas);
    }
